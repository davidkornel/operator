	...
	clust := &cluster.Cluster{
		...
		LbConfig: &cluster.Cluster_RingHashLbConfig_{
			RingHashLbConfig: &cluster.Cluster_RingHashLbConfig{
				MinimumRingSize: &wrappers.UInt64Value{Value: 4},
				MaximumRingSize: &wrappers.UInt64Value{Value: 4},
...
func makeEndpointIngress(udpClust udpCluster) *endpoint.ClusterLoadAssignment {
	...
	m, err := structpb.NewStruct(map[string]interface{}{
		"hash_key": fmt.Sprintf("%s", udpClust.callId),
	})
	if err != nil {
		log.Fatalf("Error occured while running stuctpb.NewStruct")
	}
	var lbStruct = map[string]*_struct.Struct{"envoy.lb": m}
	...
	return &endpoint.ClusterLoadAssignment{
		...
		Endpoints: []*endpoint.LocalityLbEndpoints{{
			LbEndpoints: []*endpoint.LbEndpoint{{
				...
				Metadata: &core.Metadata{FilterMetadata: lbStruct},
			}},
		}},
	}
...
func updateConfig(pod string, l *Logger) {
	...
	if err := snapshot.Consistent(); err != nil {
		l.Error(err, "snapshot inconsistency: %+v\n%+v", snapshot)
		os.Exit(1)
	}
	...
	if err := cache.SetSnapshot(pod, snapshot); err != nil {
		l.Error(err, "snapshot error %q for %+v", snapshot)
		os.Exit(1)
	}
